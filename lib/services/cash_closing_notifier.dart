...
class CashClosingNotifier {
  ...
  static CashClosingNotifier get instance =>
      _instance ??= CashClosingNotifier._();

  CashClosingNotifier._();

  StreamSubscription<String>? _eventBusSub;
  ...
  Future<void> init() async {
    ...
    _eventBusSub?.cancel();
    await _pollLatestClosings(shopId, reason: 'init');
    _eventBusSub = EventBus().stream.listen((event) {
      ...
      if (event == EventBus.dataRefresh ||
          event == 'sync_now_completed' ||
          event == 'app_resumed' ||
          event == 'cash_closings_changed') {
        unawaited(_pollLatestClosings(shopId, reason: event));
      }
    });
  ...
  Future<void> _pollLatestClosings(
    String shopId, {
    ...
    try {
      _fetchCount += 1;
      debugPrint(
        '[SYNC][FETCH] collection=cash_closings count=$_fetchCount reason=$reason limit=7',
      );
