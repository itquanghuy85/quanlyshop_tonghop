...
class _FinancialReportViewState extends State<FinancialReportView>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      for (final sale in sales) {
        ...
        // Lợi nhuận thực theo kỳ: ghi nhận theo giá bán và giá vốn, không phụ thuộc đã thu/chi tiền.
        recognizedRevenue += sale.finalPrice;
        if (sale.totalCost > 0) {
        ...
        // NOTE: Dòng tiền chỉ ghi nhận khi có tiền thực thu/chi.
        // Lợi nhuận thực được tính riêng ở recognizedRevenue/recognizedCost phía trên.
      }
      ...
      for (final repair in repairs) {
        recognizedRevenue += repair.price;
        if (repair.totalCost > 0) {
          recognizedCost += repair.totalCost;
        }

        if (repair.paymentMethod.toUpperCase() == 'CÔNG NỢ') {
          // Chưa thu tiền khách thì chưa ghi nhận THU TIỀN vào dòng tiền.
          // Nhưng lợi nhuận thực đã được ghi nhận phía trên theo giá/giá vốn.
          continue;
        }
        ...
        // NOTE: Dòng tiền chi linh kiện theo logic nhập/trả nợ riêng;
        // lợi nhuận thực dùng repair.totalCost ở phần recognizedCost.
      }
      ...
      for (final e in expenses) {
        if ((e['deleted'] ?? 0) != 1) {
          final expenseDate =
              (e['date'] as int?) ?? (e['createdAt'] as int?) ?? 0;
      ...
      final debtPayments = await _db.getDebtPaymentsWithDebtInfoByDateRange(
        startMs,
        endMs,
      );
      ...
      for (final imp in imports) {
        ...
        final importDate =
            (imp['importDate'] as int?) ?? (imp['createdAt'] as int?) ?? 0;
        ...
        if (!hasMatchingExpense && amount > 0) {
          allTransactions.add(
            TransactionItem(
              ...
              category: 'Nhập hàng',
              description:
                  '${imp['productName'] ?? 'Sản phẩm'} - ${imp['supplierName'] ?? 'NCC'}',
              amount: amount,
              isIncome: false,
      ...
      final supplierPays = await _db.getSupplierPaymentsByDateRange(
        startMs,
        endMs,
      );
      ...
      final partnerPays = await _db.getRepairPartnerPaymentsByDateRange(
        startMs,
        endMs,
      );
        ...
        if (amount > 0) {
          allTransactions.add(
            TransactionItem(
              ...
              description:
                  'Thanh toán đối tác: ${pp['partnerName'] ?? 'Đối tác'}',
              amount: amount,
              isIncome: false,
      ...
      setState(() {
        ...
        _realProfit = recognizedRevenue - recognizedCost;
        _loading = false;
      });
  ...
  Widget _directionChip(String label, int index) {
    final isSelected =
        (index == 0 && !_showIncomeOnly && !_showExpenseOnly) ||
    ...
    if (!_hasPermission) {
      ...
      return Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: Text(
            AppLocalizations.of(context)?.financialReportLabel ??
                'Financial Report',
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                _directionChip(
                  AppLocalizations.of(context)?.all ?? 'Tất cả',
                  0,
                ),
                const SizedBox(width: 4),
                _directionChip(
                  AppLocalizations.of(context)?.income ?? 'Thu',
                  1,
                ),
                const SizedBox(width: 4),
                _directionChip(
                  AppLocalizations.of(context)?.expense ?? 'Chi',
                  2,
                ),
                const Spacer(),
                IconButton(
  ...
  Widget _buildSummaryCard() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            child: _summaryItem('Thu', _totalIncome, Icons.arrow_downward),
          ),
          Container(width: 1, height: 36, color: Colors.white30),
          Expanded(
            child: _summaryItem('Chi', _totalExpense, Icons.arrow_upward),
          ),
          Container(width: 1, height: 36, color: Colors.white30),
          Expanded(
            child: Column(
              children: [
                ...
                Text(
                  MoneyUtils.formatCompactCurrency(net),
                  style: TextStyle(
                    color: Colors.white,
                ...
                const SizedBox(height: 2),
                Text(
                  'LN thực: ${MoneyUtils.formatCompactCurrency(_realProfit)}',
                  maxLines: 1,
                  ...
                  Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Text(
                          '${t.isIncome ? "+" : "-"}${MoneyUtils.formatCompactCurrency(t.amount)}',
                          style: TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.bold,
                            fontSize: AppTextStyles.subtitle1.fontSize,
