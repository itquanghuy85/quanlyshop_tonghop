...
class _FastInventoryInputViewState extends State<FastInventoryInputView>
  ...
  bool get _isAccessoryOrLinhKien =>
      typeCtrl.text == 'ACCESSORY' || typeCtrl.text == 'LINHKIEN';
  ...
  bool _validateForm() {
    ...
    if (colorCtrl.text.isEmpty) {
      NotificationService.showSnackBar(
        "Vui lòng nhập màu/thông tin!",
        color: AppColors.error,
      );
      return false;
    }
    if (quantityCtrl.text.isEmpty ||
        int.tryParse(quantityCtrl.text) == null ||
    ...
    if (costCtrl.text.isEmpty ||
        CurrencyTextField.parseValueWithMultiply(costCtrl.text) <= 0) {
      NotificationService.showSnackBar(
  ...
  Future<void> _saveProduct() async {
    ...
    try {
      // Tạo tên sản phẩm từ các trường đã nhập
      String productName;
      if (_isAccessoryOrLinhKien) {
        productName = "${brandCtrl.text} ${colorCtrl.text}".trim();
      } else {
        productName =
            "${brandCtrl.text} ${modelCtrl.text} ${capacityCtrl.text} ${colorCtrl.text}"
                .trim();
      }
      ...
      final productData = {
        ...
        'model': modelCtrl.text.isNotEmpty ? modelCtrl.text : brandCtrl.text,
        ...
        'cost': CurrencyTextField.parseValueWithMultiply(costCtrl.text),
        'price': priceCtrl.text.isNotEmpty
            ? CurrencyTextField.parseValueWithMultiply(priceCtrl.text)
            : 0,
  ...
  void _applyTemplate(Map<String, dynamic> template) {
    setState(() {
      ...
      _costController.text = CurrencyTextField.formatDisplay(
        template['cost'] as int? ?? 0,
      );
      _retailController.text = CurrencyTextField.formatDisplay(
        template['retail'] as int? ?? 0,
      );
    });
    NotificationService.showSnackBar(
    ...
    try {
      final generatedSKU = await _controller.generateSKU(
        ...
        model: _modelController.text.trim().isNotEmpty
            ...
            : null,
        info: _infoController.text.trim().isNotEmpty
