...
class _CustomerListViewState extends State<CustomerListView> {
  ...
  Future<void> _refresh() async {
    ...
    for (var r in allRepairs) {
      ...
      if (phone.isNotEmpty) {
        ...
        customerMap[key]!['totalSpent'] =
            (customerMap[key]!['totalSpent'] as int) + r.price;
        customerMap[key]!['repairCount'] =
            (customerMap[key]!['repairCount'] as int) + 1;
      }
    }
    ...
    for (var s in allSales) {
      ...
      if (phone.isNotEmpty) {
        ...
        customerMap[key]!['totalSpent'] =
            (customerMap[key]!['totalSpent'] as int) + s.totalPrice;
        customerMap[key]!['saleCount'] =
            (customerMap[key]!['saleCount'] as int) + 1;
      }
    }
    ...
    if (_showUnassignedOnly) {
      ...
      result = unassigned.where((c) => customerMap.containsKey(c['phone'])).map(
        (c) {
  ...
  Future<void> _deleteSelectedCustomers() async {
    ...
    try {
      ...
      messenger.showSnackBar(
        SnackBar(
          content: Text("Đã xóa ${selectedCustomers.length} khách hàng"),
        ),
      );
    } catch (e) {
      ...
      appBar: AppBar(
        ...
        actions: _isSelectionMode
            ...
            : [
                IconButton(
                  icon: Icon(
                    _showUnassignedOnly ? Icons.group : Icons.group_off,
                    color: Colors.white,
                  ),
                  onPressed: () {
                  ...
                  tooltip: _showUnassignedOnly
                      ? "Xem tất cả khách hàng"
                      : "Xem khách chưa gán shop",
                ),
              ],
      ),
      body: _isLoading
  ...
  Widget _customerCard(Map<String, dynamic> c, int index) {
    final bool canDelete =
        _isAdmin &&
        (c['repairCount'] as int? ?? 0) == 0 &&
        (c['saleCount'] as int? ?? 0) == 0;
    final bool isSelected = _selectedIndices.contains(index);
    final int repairCount = c['repairCount'] as int? ?? 0;
    final int saleCount = c['saleCount'] as int? ?? 0;
    final int totalSpent = c['totalSpent'] as int? ?? 0;
    ...
    return Card(
      ...
      child: InkWell(
        ...
        onLongPress: _isSelectionMode ? null : () => _startSelection(index),
        borderRadius: BorderRadius.circular(8),
        child: Padding(
          padding: const EdgeInsets.all(10),
          child: Column(
            ...
            children: [
                  ...
                  Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Text(
                          MoneyUtils.formatVND(totalSpent),
                          style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 11,
              ...
              Wrap(
                ...
                children: [
                  if (repairCount > 0)
                    _compactChip('🔧 $repairCount sửa', Colors.orange.shade100),
                  if (saleCount > 0)
                    _compactChip('📱 $saleCount mua', Colors.pink.shade100),
                  if (repairCount == 0 && saleCount == 0)
                    _compactChip('Mới', Colors.blue.shade100),
                ],
              ),
            ],
          ),
  ...
  void _showCustomerFullHistory(Map<String, dynamic> c) async {
    ...
    final repairHistory = allRepairs
        .where((r) => r.phone == c['phone'])
        .toList();
  ...
  Widget _buildRepairHistoryList(List<Repair> list) {
    if (list.isEmpty)
      return const Center(child: Text("Chưa có lịch sử sửa chữa"));
  ...
  Future<void> _confirmDeleteCustomer(Map<String, dynamic> c) async {
    if (!_isAdmin) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Chỉ tài khoản QUẢN LÝ mới được xóa khách hàng khỏi danh sách',
          ),
        ),
      );
      return;
    ...
    if (hasHistory) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Không thể xóa khách đã có lịch sử sửa/bán.'),
        ),
      );
      return;
    }
  ...
  Color _getStatusColor(int status) {
    switch (status) {
      case 1:
        return Colors.orange; // Đang sửa
      case 2:
        return Colors.blue; // Chờ linh kiện
      case 3:
        return Colors.green; // Hoàn thành
      case 4:
        return Colors.red; // Đã trả máy
      default:
        return Colors.grey;
    }
  }
